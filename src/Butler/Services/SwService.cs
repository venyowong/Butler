...
namespace Butler.Services
{
    ...
    public static class SwService
    {
        ...
        public static async Task<List<IndexConstituent>> GetIndexConstituents(string code)
        {
            using (var client = new HttpClient())
            {
        ...
        public static async Task<Dictionary<string, string>> GetStockIndustryMap()
        {
            var stockIndustryMap = new Dictionary<string, string>();
            foreach (var indexCode in IndustryMap.Keys)
            {
                var constituents = await DataService.GetIndexConstituents(indexCode);
                constituents?.ForEach(x => stockIndustryMap.Add(x.StockCode, indexCode));
            }
            return stockIndustryMap;
        }

        public static async Task<Dictionary<string, List<IndexQuotation>>> GetIndexQuotationsInTimeRange(DateTime begin, DateTime end)
        {
            var result = new Dictionary<string, List<IndexQuotation>>();
            foreach (var indexCode in IndustryMap.Keys)
            {
                result.Add(indexCode, await DataService.GetIndexQuotations(indexCode, begin, end));
            }
            return result;
        }
    }
