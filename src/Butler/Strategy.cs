...
namespace Butler
{
    public static class Strategy
    {
        ...
        /// <returns></returns>
        public static async Task<double?> GetAlphaBasedOnIndustry(string fundCode)
        {
            ...
            try
            {
                ...
                var assetConfig = DataService.GetFundAssetConfigs(fundCode)?.OrderByDescending(x => x.Date).FirstOrDefault();
                if (assetConfig == null)
                {
                ...
                #region 基础数据获取
                var reportDate = DateTimeHelper.GetLastAnnualReportDate();
                var positions = await DataService.GetFundStockPositions(fundCode, reportDate);
                ...
                var stockIndustryMap = await SwService.GetStockIndustryMap();
                var industryQuotationMap = await SwService.GetIndexQuotationsInTimeRange(firstNavDate, DateTime.Now);
                #endregion
        ...
        #region 择时能力
        public static async Task<double?> GetFundTimingAbility(string fundCode)
        {
            #region 基础条件校验
            var info = DataService.GetFundInfo(fundCode);
            if (info == null)
            ...
            var exposures = await GetFundStyleExposures(fundCode, begin.AddMonths(-2), end);
            var inflections = Market.GetStyleInflections()
        ...
        private static double GetOperationScore(StyleInflection inflection, List<FundStyleExposure> exposures, DateTime end)
        {
            double? r = null;
        ...
        private static double? GetExposure(string index, FundStyleExposure exposure)
        {
            switch (index)
            {
                case "CI005917":
        ...
        private static double GetOperationScore(DateTime date1, DateTime date2)
        {
            if (date2 < date1)
            {
                return 3;
        ...
        private static async Task<FundStyleExposure> GetStyleExposure(string fundCode, DateTime end)
        {
